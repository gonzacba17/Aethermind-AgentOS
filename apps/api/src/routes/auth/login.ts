...
router.post('/login', authLimiter, async (req: Request, res: Response) => {
  try {
    ...
    if (!clientAccessToken) {
      ...
      const newClientTokenHash = await bcrypt.hash(newClientToken, 10);
      const newClientTokenPrefix = newClientToken.slice(0, 16);
      ...
      await db.transaction(async (tx) => {
        ...
        await tx.insert(clients).values({
          ...
          accessTokenHash: newClientTokenHash,
          accessTokenPrefix: newClientTokenPrefix,
          sdkApiKey,
          sdkApiKeyHash,
          sdkApiKeyPrefix,
