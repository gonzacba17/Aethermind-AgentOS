# 🔍 BACKEND AUDIT V2 — Aethermind AgentOS

> **Date:** 2026-02-01  
> **Scope:** Full-stack audit — Security, Error Handling, Testing, Infrastructure, Frontend Integration, Technical Debt  
...
9. [Top 3 Files to Refactor](#9-top-3-files-to-refactor)
10. [3-Week Action Plan](#10-3-week-action-plan)
...
- 🟡 Medium: 9 findings (✅ 6 FIXED)
- 🟢 Low: 5 findings (✅ 1 FIXED)

> **Last Updated:** 2026-02-17 — Security hardening session completed (Week 1)
...
### 7.1 🟡 Duplicate Auth Logic

Three files implement overlapping authentication:

| File                       | Purpose                                    |
| -------------------------- | ------------------------------------------ |
| `middleware/auth.ts`       | JWT + API key validation for routes        |
| `middleware/apiKeyAuth.ts` | API key validation for ingestion           |
| `utils/auth-helpers.ts`    | Token generation, verification, extraction |
| `middleware/jwt-auth.ts`   | Another JWT validation middleware          |

This creates confusion about which middleware to use where, and risks inconsistencies (e.g., `auth.ts` reads `JWT_SECRET` directly, while `auth-helpers.ts` uses a cached getter with validation).
...
| 7.1  | Duplicate Auth Logic          | 🟡 Medium   | Medium     | Inconsistency      | P2       | ⬜ Open                   |
...
### 🥈 #2: `apps/api/src/routes/auth.ts` (1228 lines)

**Why:** Largest file in the codebase. Handles signup, login, OAuth, password reset, email verification, plan updates, temporary user conversion, and cross-domain sessions — all in one router. Extremely hard to test, maintain, or onboard to.

**Refactoring plan:**

1. Extract into sub-modules:
   - `auth/signup.ts` — User registration
   - `auth/login.ts` — Email/password login
   - `auth/oauth.ts` — Google/GitHub OAuth
   - `auth/password-reset.ts` — Forgot/reset password
   - `auth/verify.ts` — Email verification
   - `auth/session.ts` — Cross-domain session management
   - `auth/plan.ts` — Plan updates and temporary user conversion
2. Create shared validation schemas (Zod)
3. Add comprehensive unit + integration tests for each sub-module
4. Remove inline `try-catch` blocks in favor of `asyncHandler`
...
| Wed | Begin `auth.ts` decomposition — extract signup + login | new route files              | 4h         | ⬜ Pending                  |
| Wed | Extract OAuth routes                                   | new route file               | 3h         | ⬜ Pending                  |
| Thu | Extract password reset + email verification            | new route files              | 3h         | ⬜ Pending                  |
...
| Thu | Consolidate auth middleware (remove duplicates)              | `auth.ts`, `jwt-auth.ts`, `auth-helpers.ts` | 4h         | ⬜ Pending                  |
...
| Fri | Final review, update BACKEND_AUDIT status                    | this file                                   | 2h         | ⬜ Pending                  |
...
| 2         | Error Handling & Testing             | ~28h       | ~4h ✅          |
| 3         | Dashboard, Tech Debt & Observability | ~25h       | ~2h ✅          |
| **Total** |                                      | **~76.5h** | **~26.5h done** |

> **Progress as of 2026-02-17:** 14 of 30 findings resolved. All P0/Critical items FIXED. Week 1 nearly complete (only code review remaining).
...
| `apps/api/src/routes/auth.ts`                                       | 1228    | Reviewed  |
