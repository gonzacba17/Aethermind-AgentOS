...
import { useUserProfile, useTestSDKConnection } from "@/hooks/api/useUserProfile"
...
export function SDKConnectCard() {
  ...
  const handleTestConnection = async () => {
    ...
    try {
      await testConnection.mutateAsync(sdkApiKey)
        ...
        <div className="p-4 rounded-xl bg-card border border-border">
              ...
              {sdkApiKey && (
                <>
                  ...
                  <Button
                    variant="ghost"
                    size="sm"
                    onClick={() => handleCopy(sdkApiKey, 'SDK API Key')}
                  >
                    {copied === 'SDK API Key' ? <Check className="h-4 w-4" /> : <Copy className="h-4 w-4" />}
                  </Button>
                </>
              )}
            </div>
          ...
          {sdkApiKey ? (
            ...
            <div className="p-3 rounded-lg bg-muted/30 border text-sm text-muted-foreground">
              No SDK key found — key is generated automatically on signup.
            </div>
          )}
        </div>
